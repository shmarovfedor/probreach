...
void print_help()
{
  ...
  cout << "-u - maximum depth of every simulation path (default = " << max_depth
       << ")" << endl;
  cout << "-p - maximum number of simulation paths (default = " << max_paths
       << ")" << endl;
  cout << "-n - number of points used in IVP solving (default = " << num_points
       << ")" << endl;
...
void parse_cmd(int argc, char *argv[])
{
  ...
  for (int i = 1; i < argc; i++)
  {
    // filename
    if (
      string(argv[i]).substr(string(argv[i]).find_last_of('.') + 1) == "pdrh" ||
