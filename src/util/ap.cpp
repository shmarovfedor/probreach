...
int ap::jumps_per_mode(pdrh::mode *m, vector<box> boxes)
{
  ...
  for (pdrh::state st : pdrh::goal)
  {
    ...
    {
      return ceil(
        (ap::get_meal_time(st.prop, boxes) / sample_rate).rightBound());
    }
  }
  return ceil((ap::get_meal_time(m, boxes) / sample_rate).rightBound());
}

int ap::jumps_per_mode(
  pdrh::mode *cur_mode,
  pdrh::mode *prev_mode,
  vector<box> boxes)
{
  ...
  if (prev_mode->id > 0)
  {
    ...
    left_over = capd::interval(
      fmod(meal_time_prev_mode.leftBound(), sample_rate_prev_mode.leftBound()),
      fmod(
        meal_time_prev_mode.rightBound(), sample_rate_prev_mode.rightBound()));
  }
  else
  {
  ...
  for (pdrh::state st : pdrh::goal)
  {
    ...
    {
      return ceil(
        ((ap::get_meal_time(st.prop, boxes) + left_over) / sample_rate_cur_mode)
          .rightBound());
    }
  }
  return ceil(
    ((ap::get_meal_time(cur_mode, boxes) + left_over) / sample_rate_cur_mode)
      .rightBound());
}
  ...
  vector<box> boxes)
{
  ...
  {
    if (
      (pdrh::par_map.find(it->first) == pdrh::par_map.end()) &&
...
int ap::simulate_path(vector<pdrh::mode *> path, box init, vector<box> boxes)
{
  ...
  if (path.size() == 1)
  {
    return decision_procedure::check_invariants(
      path.front(),
      time_to_goal(path.front(), boxes),
      init,
      boxes,
      global_config.solver_bin,
      global_config.solver_opt);
  }
  // reachability depth > 0
  vector<box> sol_box;
  ...
  for (size_t j = 0; j < path.size() - 1; j = j + window_size)
  {
    ...
    for (size_t i = j; (i < j + window_size) && (i < path.size() - 1); i++)
    {
      ...
      CLOG_IF(global_config.verbose, INFO, "algorithm")
        << "Init box (VERIFIED) in mode " << cur_mode->id << " for depth " << i;
      ...
      {
        invt_check = decision_procedure::check_invariants(
          cur_mode,
          time,
          init_box_hull,
          boxes,
      ...
      case decision_procedure::SAT:
        CLOG_IF(global_config.verbose, INFO, "algorithm")
          << "Invariant is SAT in mode " << cur_mode->id << " @ depth=" << i;
        break;

      case decision_procedure::UNDET:
        CLOG_IF(global_config.verbose, INFO, "algorithm")
          << "Invariant is UNDET in mode " << cur_mode->id << " @ depth=" << i;
        return decision_procedure::UNDET;

      case decision_procedure::UNSAT:
        CLOG_IF(global_config.verbose, INFO, "algorithm")
          << "Invariant is UNSAT in mode " << cur_mode->id << " @ depth=" << i;
      ...
      {
        sol_box.push_back(
          solve_odes(cur_mode->odes, init_box.at(k), time, boxes));
        //sol_box.push_back(solve_odes_nonrig(cur_mode->odes, init_box.at(k), time, boxes));
      }
      CLOG_IF(global_config.verbose, INFO, "algorithm")
        << "Solution (VERIFIED) box in mode " << cur_mode->id
        << " at depth = " << i;
      CLOG_IF(global_config.verbose, INFO, "algorithm")
        << box_factory::box_hull(sol_box);
      //            cout << "===========" << endl;
      ...
      for (box b : sol_box)
      {
        ...
        part_sol_box.insert(
          part_sol_box.end(), part_boxes.begin(), part_boxes.end());
      }

      // resetting the initial state for the next mode
      init_box.clear();
      map<string, pdrh::node *> reset_map =
        cur_mode->get_jump(next_mode->id).reset;
      for (box b : part_sol_box)
      {
        ...
        init_box.push_back(
          ap::apply_reset(cur_mode->get_jump(next_mode->id).reset, b, boxes));
      }
  ...
  case decision_procedure::SAT:
    CLOG_IF(global_config.verbose, INFO, "algorithm")
      << "Invariant is SAT in mode " << path.back()->id
      << " @ depth=" << path.size() - 1;
    break;

  case decision_procedure::UNDET:
    CLOG_IF(global_config.verbose, INFO, "algorithm")
      << "Invariant is UNDET in mode " << path.back()->id
      << " at depth=" << path.size() - 1;
    return decision_procedure::UNDET;

  case decision_procedure::UNSAT:
    CLOG_IF(global_config.verbose, INFO, "algorithm")
      << "Invariant is UNSAT in mode " << path.back()->id
      << " at depth=" << path.size() - 1;
    return decision_procedure::UNSAT;
  }
  ...
  vector<string> obj_name)
{
  ...
  {
    box sol = solve_odes(
      path.front()->odes, init, time_to_goal(path.front(), boxes), boxes);
  ...
  for (size_t j = 0; j < path.size() - 1; j = j + window_size)
  {
    ...
    for (size_t i = j; (i < j + window_size) && (i < path.size() - 1); i++)
    {
      ...
      cout << "Solution @ step " << i << " in mode " << cur_mode->id << ": "
           << endl
           << sol << endl;
...
#pragma omp critical
      ...
      for (auto it = reset_map.begin(); it != reset_map.end(); it++)
      {
        if (
          ...
          (pdrh::dd_map.find(it->first) == pdrh::dd_map.end()))
        {
          init_map.insert(make_pair(
            it->first, pdrh2box::node_to_interval(it->second, reset_boxes)));
        }
      }
      // can add random error here
      init = box(init_map);
  ...
  cout << "Final solution at mode " << path.back()->id << ": " << endl
       << sol << endl;
  map<string, capd::interval> obj_map, b_map;
  b_map = sol.get_map();
  for (auto it = b_map.begin(); it != b_map.end(); it++)
...
capd::interval
ap::compute_robustness(vector<pdrh::mode *> path, box init, vector<box> boxes)
{
  ...
  if (path.size() == 1)
  {
    ...
    for (pdrh::node *n : path.front()->invts)
    {
      ...
      if (
        n->value != ">=" || n->operands.size() > 2 ||
        n->operands.back()->value != "0")
      {
        ostringstream s;
        s << "Please rewrite the invariant \"" << pdrh::node_to_string_infix(n)
          << "\" in supported format";
        throw invalid_argument(s.str());
      }
      //            cout << "Representation: OK" << endl;
      capd::interval rob =
        pdrh2box::node_to_interval(n->operands.front(), {sol, boxes});
  ...
  for (size_t j = 0; j < path.size() - 1; j = j + window_size)
  {
    ...
    for (size_t i = j; (i < j + window_size) && (i < path.size() - 1); i++)
    {
      ...
      // solving odes
      //sol = solve_odes_nonrig(cur_mode->odes, init, time, boxes);
      sol = solve_odes_discrete(
        cur_mode->odes, init, time, global_config.ode_discretisation, boxes);
      //sol = init;
      ...
      for (pdrh::node *n : cur_mode->invts)
      {
        ...
        {
          ostringstream s;
          s << "Please rewrite the invariant \""
            << pdrh::node_to_string_infix(n) << "\" in supported format";
          throw invalid_argument(s.str());
        }
        //                cout << "Representation: OK" << endl;
        capd::interval rob =
...
capd::interval ap::compute_max_robustness(
...
capd::interval ap::compute_min_robustness(
  vector<vector<pdrh::mode *>> paths,
  box init,
  vector<box> boxes)
{
...
vector<vector<pdrh::mode *>> ap::get_all_paths(vector<box> boxes)
{
  ...
  for (pdrh::state i : pdrh::init)
  {
    for (pdrh::state g : pdrh::goal)
    {
      ...
      vector<pdrh::mode *> path =
        pdrh::get_shortest_path(pdrh::get_mode(i.id), pdrh::get_mode(g.id));
      if (path.size() > 0)
      {
        if (path.size() == 1)
        {
...
{
  return jump.guard->value == "=" && jump.guard->operands.size() == 2 &&
...
int ap::verify(size_t min_depth, size_t max_depth, vector<box> boxes)
{
  ...
  vector<vector<pair<int, box>>> paths = {
    {make_pair(pdrh::init.front().id, pdrh2box::init_to_box(boxes))}};
  // global time
  ...
  while (paths.size() > 0)
  {
    ...
    //cout << "====================" << endl;
    CLOG_IF(global_config.verbose, INFO, "algorithm")
      << "Mode " << cur_mode->id << " Step " << (path.size() - 1);
    ...
    if (
      global_time >= pdrh2box::node_to_interval(
                       pdrh::var_map[global_config.global_time].second) ||
      capd::intervals::intersection(
        global_time,
        pdrh2box::node_to_interval(
          pdrh::var_map[global_config.global_time].second),
        glob_time_intersection) ||
      path.size() - 1 >= max_depth)
    {
      CLOG_IF(global_config.verbose, INFO, "algorithm")
        << "Global time limit has been reached in mode " << cur_mode->id;
      return decision_procedure::SAT;
    }
    ...
    if (cur_mode->jumps.size() == 0)
    {
      ...
      capd::interval time_bound =
        pdrh2box::node_to_interval(cur_mode->time.second);
      if (
        global_time + time_bound >
        pdrh2box::node_to_interval(
          pdrh::var_map[global_config.global_time].second))
      {
        time_bound = pdrh2box::node_to_interval(
                       pdrh::var_map[global_config.global_time].second) -
                     global_time;
      }
      CLOG_IF(global_config.verbose, INFO, "algorithm")
      ...
      int invt_check = decision_procedure::check_invariants(
        cur_mode,
        time_bound,
        init,
        boxes,
    ...
    for (pdrh::mode::jump jump : cur_mode->jumps)
    {
      ...
      pair<capd::interval, box> jump_time =
        decision_procedure::get_jump_time(cur_mode, jump, init, boxes);
      CLOG_IF(global_config.verbose, INFO, "algorithm")
        << "Time of the jump to mode " << jump.next_id << " : "
        << jump_time.first << "; witness: " << jump_time.second;
      //cout << "===============" << endl;
      // adding only those jumps which are enabled within the mode
      if (jump_time.first != capd::interval(-1))
      {
        CLOG_IF(global_config.verbose, INFO, "algorithm")
          << "Checking invariants in mode " << cur_mode->id;
        //cout << "Initial condition: " << init << endl;
        int invt_check = decision_procedure::check_invariants(
          cur_mode,
          jump_time.first,
    ...
    for (auto it = jumps_times.begin(); it != jumps_times.end(); it++)
    {
      ...
      if (capd::intervals::intersection(
            sample_jump.second.second.get_map()[global_config.global_time],
            sol.get_map()[global_config.global_time],
            intersection))
      {
        // considering box hull here then
        sol = ap::apply_reset(
          cur_mode->get_jump(sample_jump.first).reset,
          box_factory::box_hull({sol, sample_jump.second.second}),
          boxes);
      }
...
int ap::simulate(size_t min_depth, size_t max_depth, vector<box> boxes)
{
  ...
  while (paths.size() > 0)
  {
    ...
    capd::interval integration_step =
      pdrh2box::node_to_interval(cur_mode->time.second).rightBound() /
      global_config.ode_discretisation;
    //cout << "Integration step: " << integration_step << endl;
    // represents a jump due to sampling
    pair<int, pair<capd::interval, box>> sample_jump =
      make_pair(0, make_pair(capd::interval(0.0), box()));
    for (size_t i = 0; i < global_config.ode_discretisation; i++)
    {
      CLOG_IF(global_config.verbose, INFO, "algorithm")
        << "Checking invariants in mode " << cur_mode->id << " Step "
        << path.size() - 1;
      CLOG_IF(global_config.verbose, INFO, "algorithm")
      ...
      if (
        init.get_map()[global_config.global_time].leftBound() >=
          pdrh2box::node_to_interval(
            pdrh::var_map[global_config.global_time].second)
            .rightBound() ||
        path.size() - 1 >= max_depth)
      ...
      box sol =
        solve_odes_discrete(cur_mode->odes, init, integration_step, 1, boxes);
      ...
      for (pdrh::mode::jump jump : cur_mode->jumps)
      {
        if (ap::is_sample_jump(jump))
        {
          ...
          {
            sample_jump = make_pair(
              jump.next_id,
              make_pair(cur_time, ap::apply_reset(jump.reset, sol, boxes)));
      ...
      for (pdrh::mode::jump jump : cur_mode->jumps)
      {
        ...
        if (
          jumps_times.find(jump.next_id) == jumps_times.end() &&
          !ap::is_sample_jump(jump))
        {
          ...
          if (pdrh2box::check_zero_crossing(jump.guard, boxes, init, sol))
          {
            ...
            {
              sol = ap::apply_reset(
                cur_mode->get_jump(sample_jump.first).reset, sol, boxes);
            }
            // applying the corresponding reset
            jumps_times.insert(make_pair(
              jump.next_id,
              make_pair(cur_time, ap::apply_reset(jump.reset, sol, boxes))));
            //cout << ". Enabled at or before " << cur_time << endl;
          }
    ...
    if (jumps_times.empty())
    {
      ...
      if (sample_jump.first != 0)
        jumps_times.insert(sample_jump);
    }
