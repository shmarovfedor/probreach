...
box rnd::get_sobol_sample(gsl_qrng *q, box b)
{
  ...
  for (auto it = b_edges.cbegin(); it != b_edges.cend(); it++)
  {
    edges.insert(make_pair(
      it->first,
      capd::interval(
        it->second.leftBound() + v[i] * capd::intervals::width(it->second))));
    i++;
...
box rnd::get_random_sample2(gsl_rng *r, box b, int k, double non)
{
  ...
  if (k == 2)
  {
    for (auto it = pdrh::rv_map.cbegin(); it != pdrh::rv_map.cend(); it++)
    {
        ...
        pdrh::distribution::uniform.cend())
      {
        //std::cout<<"unif!"<<endl;
        edges.insert(make_pair(
          it->first, vect[1] + gsl_rng_uniform(r) * (vect[0] - vect[1])));
      }
      else if (
        pdrh::distribution::normal.find(it->first) !=
        pdrh::distribution::normal.cend())
      {
        edges.insert(make_pair(
          it->first, vect[1] + gsl_ran_gaussian_ziggurat(r, vect[0])));
      }
      else
      {
  ...
  if (k == 1)
  {
    for (auto it = pdrh::rv_map.cbegin(); it != pdrh::rv_map.cend(); it++)
    {
      ...
      {
        edges.insert(
          make_pair(it->first, non + gsl_rng_uniform(r) * (vect[0] - non)));
      }
      else if (
        pdrh::distribution::normal.find(it->first) !=
        pdrh::distribution::normal.cend())
      {
        edges.insert(
          make_pair(it->first, non + gsl_ran_gaussian_ziggurat(r, vect[0])));
      }
      else
      {
        cerr << "Distribution is not supported ny Random number generator\n";
...
box rnd::get_beta_random_sample(gsl_rng *r, box alpha, box beta, box b)
{
  ...
  {
    double beta_dist_value = gsl_ran_beta(
      r,
      alpha.get_map()[it->first].leftBound(),
      beta.get_map()[it->first].leftBound());
    edges.insert(make_pair(
      it->first,
      capd::interval(
        it->second.leftBound() +
        beta_dist_value * capd::intervals::width(it->second))));
  }
  return box(edges);
}
...
{
  return log(x) - 1 / (2 * x) - 1 / (12 * pow(x, 2)) + 1 / (120 * pow(x, 4)) -
...
pair<box, box>
rnd::update_beta_dist(vector<box> q, box domain, box prev_alpha, box prev_beta)
{
  vector<box> q_norm;
  for (box b : q)
  {
    ...
    for (auto it = b_map.cbegin(); it != b_map.cend(); it++)
    {
      l_map.insert(make_pair(
        it->first, capd::interval(domain.get_map()[it->first].leftBound())));
      w_map.insert(make_pair(
        it->first, capd::intervals::width(domain.get_map()[it->first])));
    }
    box w(w_map), l(l_map);
  ...
  {
    pair<double, double> beta_params = rnd::solve_beta_system(
      c1.get_map()[it->first].mid().leftBound(),
...
pair<double, double> rnd::solve_beta_system(
  double c1,
  double c2,
  double prev_alpha,
  double prev_beta)
