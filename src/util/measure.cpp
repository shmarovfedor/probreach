...
capd::interval measure::p_measure(box b, double e)
{
  ...
  {
    if (
      global_model.declarations.rv_map.find(it->first) !=
      global_model.declarations.rv_map.cend())
    {
      res *= measure::integral(
               it->first,
               std::get<0>(global_model.declarations.rv_map[it->first])->to_infix(),
               it->second,
               measure::precision(e, edges.size()))
               .first;
    }
    else if (
      global_model.declarations.dd_map.find(it->first) !=
      global_model.declarations.dd_map.cend())
    {
      bool measure_exists = false;
      map<node *, node *> tmp_map = global_model.declarations.dd_map[it->first];
...
capd::interval measure::get_sample_prob(box domain, box mean, box sigma)
{
  ...
  for (auto it = edges.begin(); it != edges.end(); it++)
  {
    ...
    if (
      global_model.declarations.par_map[it->first].first->value !=
      global_model.declarations.par_map[it->first].second->value)
    {
      double prec = 1e-5;
...
std::vector<box> measure::get_rv_partition()
{
  ...
  for (auto it = global_model.declarations.rv_map.cbegin();
       it != global_model.declarations.rv_map.cend();
       it++)
  {
    ...
    std::pair<capd::interval, std::vector<capd::interval>> bound =
      measure::bounds_from_pdf(
        ...
        measure::precision(
          global_config.precision_prob, global_model.declarations.rv_map.size()));
    // updating rv bounds
    global_model.declarations.rv_map[it->first] = make_tuple(
      std::get<0>(it->second),
      new node(std::to_string(bound.first.leftBound())),
      new node(std::to_string(bound.first.rightBound())),
      get<3>(it->second));
    // updating var bounds
    global_model.declarations.var_map[it->first] = make_pair(
      new node(std::to_string(bound.first.leftBound())),
      new node(std::to_string(bound.first.rightBound())));
