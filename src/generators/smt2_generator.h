...
std::string
generate_flow_invt_check(pdrh::mode *, capd::interval, box, std::vector<box>);
...
// fourth argument is the vector of samples
std::string
generate_flow_invt_check_c(pdrh::mode *, capd::interval, box, std::vector<box>);
...
std::string generate_jump_check(
  pdrh::mode *,
  std::vector<pdrh::mode::jump>,
  box,
  std::vector<box>);
